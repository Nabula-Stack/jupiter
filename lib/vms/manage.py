...
def destroy_vm(host, vmid):
    """Permanently destroys VM: powers off if running, deletes entire VM folder from datastore, removes from inventory."""
    import os
    ...
    # 3. Unregister from inventory first (so ESXi releases file locks)
    host.run(f"vim-cmd vmsvc/unregister {vmid}")
    ...
    if vmx_path:
        ...
        if vm_dir and vm_dir.startswith("/vmfs/volumes/") and len(vm_dir.split("/")) >= 5:
            ...
            if isinstance(result, str) and result.startswith("Error:"):
                return result
            return f"VM {vmid} destroyed. Folder deleted: {vm_dir}"

    return f"VM {vmid} unregistered (could not determine VM folder path)."
